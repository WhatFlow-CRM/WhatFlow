...
let translate_icon_2 = chrome.runtime.getURL("logo/prime_translation_2.png");
...
function get_message_div(message_container) {
    // First try to find the new structure with data-testid="selectable-text"
    let message_parent_div = message_container.querySelector('[data-testid="selectable-text"]');
    
    // Fallback to old structure
    if (!message_parent_div) {
        message_parent_div = message_container.querySelector('.selectable-text') || message_container.querySelector('.copyable-text');
    }
...
function replace_message(message_container) {
    if (!message_container.querySelector('[data-testid="selectable-text"]') || !message_container.querySelector('.translated_message') || !message_container.querySelector('.original_message')) 
        return;
...
function translate_visible_messages() {
    ...
    visible_messages_div.forEach(async (message_container, index) => {        
        // Check if it has valid translation data (not just if it was visited)
        if (!message_container.querySelector('.translated_message_div .detected_language_name') && message_container.querySelector('[data-testid="selectable-text"]')) {            
            // Remove old translated_message_div if it exists but is empty
            let oldTranslatedDiv = message_container.querySelector('.translated_message_div');
            if (oldTranslatedDiv) {
                oldTranslatedDiv.remove();
            }
